...
class OPOperator():
    def __init__(self, mode, filename, compiletype):
        ...
        self.prefix_dir = '%s/openssl_make' % (self.tmp_dir)
...
#--------------------------------------------------------#
    ...
    def build_component(self):
        ...
        tar_cmd = 'cd %s; mkdir -p %s; tar -zxvf %s -C %s --strip-components 1' % \
        ...
        for c_type in self.compiletype:
            if c_type == 'comm':
                ...
                self.error_handler(ret)
                config_cmd = 'cd %s/%s; ./config enable-ssl3-method enable-mdc2 enable-md2 -fPIC -shared -fstack-protector-strong -g -D_FORTIFY_SOURCE=2 -O2 -Wl,--build-id=none,-z,relro,-z,now,-z,noexecstack --prefix=%s --openssldir=%s' % (self.local_dir, source_code_path, self.prefix_dir, self.prefix_dir)
                ret = self.exe_cmd(config_cmd)
                self.error_handler(ret)
                make_cmd = 'cd %s/%s; make -j%s && make install_sw' % (self.local_dir, source_code_path, cpu_num)
                ...
                ret = self.exe_cmd(prepare_cmd)
                self.error_handler(ret)
                config_cmd = 'cd %s/%s; ./config no-shared enable-mdc2 enable-md2 -fPIE -pie -fstack-protector-strong -g -D_FORTIFY_SOURCE=2 -O2 -fPIC -Wl,--build-id=none,-z,relro,-z,now,-z,noexecstack --prefix=%s --openssldir=%s' % (self.local_dir, source_code_path, self.prefix_dir, self.prefix_dir)
                ret = self.exe_cmd(config_cmd)
                self.error_handler(ret)
                make_cmd = 'cd %s/%s; make -j%s && make install_sw' % (self.local_dir, source_code_path, cpu_num)
                ret = self.exe_cmd(make_cmd)
    ...
    def shrink_component(self):
        ...
        for c_type in self.compiletype:
            if c_type == 'comm':
                ...
                cp_cmd2 = 'cp -r %s/install/comm/lib* %s/install_comm_dist/lib' % (self.local_dir, self.local_dir)
        ...
        rm_cmd = 'rm -rf %s/install_comm_dist/lib/engines-3 %s/install_comm_dist/lib/pkgconfig %s/install_comm_dist/lib/ossl-modules %s/install_comm_dist/bin/c_rehash' % (self.local_dir, self.local_dir, self.local_dir, self.local_dir)
        self.exe_cmd(rm_cmd)
        chmod_cmd = 'chmod -x %s/install_comm_dist/lib/lib*.so.3' % (self.local_dir)
    ...
    def dist_component(self):
        ...
        for c_type in self.compiletype:
            if c_type == 'comm':
                ...
                self.exe_cmd(rm_cmd)
                cp_cmd1 = 'cp -r %s/install_comm_dist/* %s/llt/' % (self.local_dir, install_path)
                self.exe_cmd(cp_cmd1)
            elif c_type == 'release':
                print ("[WARNING] Not supported build type")
            elif c_type == 'debug':
